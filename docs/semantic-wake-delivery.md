...
| A18 | 首选失败后逐次换候选（最多 3 个）且每次留痕；**v0.1.2**：每次失败带**真实原因**、首选（触发者）多跑一轮、改投时留痕 + telegram 告警 | ✔ **线上首次实测（2026-09-17 03:23:44–03:24:19）**：`唤醒投递失败，换下一个候选` 命中 2 次（触发者 `session-5ae01805…` → `session-a5375716…`），第三条改投 `session-879c4ae1…` 成功——该路径首次被真实触发。**实测暴露三处缺陷并已修**：① 失败原因被 catch 吞掉（只剩「换下一个候选: <id>」）② 首选即刻换靶（触发者=重启发起方，其失败常是瞬时状态）③ 改投静默（触发者永不知道自己错过）。单测 `tests/wake-delivery.test.mjs` 7 项（含事故形状复刻）→ sentinel 全量 **66/66** |
...
**验收计数**：total **22** / proven **20** / pending **2**（A18 于 2026-09-17 首次线上实测并修复三处缺陷后转入 proven；余 A17 / A22 待线上验收）。
...
- **未实现 / 未验证（显式）**：
  1. ~~**候选换腿循环无单测**（A18）~~ **已消除（2026-09-17 · v0.1.2）**：编排与失败取证抽入 `src/wake-delivery.ts`（纯函数、零 IO），配 `tests/wake-delivery.test.mjs` 7 项（含事故形状复刻）；证据面见 A18。
...
- **2026-09-17 唤醒投递「换候选」首次线上触发 → 三处缺陷 + U1/U7 闭环**
